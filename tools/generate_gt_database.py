...
class GTDatabaseGenerator(KittiDataset):
    ...
    def generate_gt_database(self):
        gt_database = []
        for idx, sample_sequence in enumerate(self.image_idx_list):
            sample_id = int(sample_sequence[-6:])
            sequence = int(sample_sequence[:4])

            print('process gt sample (id=%06d) of (%04d)' % (sample_id, sequence))

            pts_lidar = self.get_lidar(sample_id, sequence)
            calib = self.get_calib(sequence)
            pts_rect = calib.lidar_to_rect(pts_lidar[:, 0:3])
            pts_intensity = pts_lidar[:, 3]

            obj_list = self.filtrate_objects(self.get_label(sample_id, sequence))
