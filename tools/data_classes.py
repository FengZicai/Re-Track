...
class Box:
    ...
    """增强版本的label class,在检测数据集的Object3d的基础上加以修改"""

    def __init__(self, box, center, size, orientation, label=np.nan, score=np.nan, velocity=(np.nan, np.nan, np.nan),
                 name=None):
        """
        ...
        # 以下内容与图像有关,先行注释,再做修改,需要传入box类
        self.src = box
        self.pos = np.array((float(box["x"]), float(box["y"]), float(box["z"])), dtype=np.float32)
        self.ry = box["rotation_y"]
    #     self.level_str = None
