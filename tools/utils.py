...
import numpy as np
from tools.external.python_plyfile.plyfile import PlyElement, PlyData
from pyquaternion import Quaternion

from tools.data_classes import PointCloud
from tools.metrics import estimateOverlap,estimateOverlapwithboxes3d
...
def getScoreIoU(a, b):
    ...
    return torch.tensor([score])#.cuda().float()
...
def regularizePC2(model_inputsize, PC):
    return regularizePC(PC=PC, model_inputsize=model_inputsize) #未见引用

def regularizePC_PointCloud(PC, model_inputsize):
    pc_intensity=np.array(PC.intensity, dtype=np.float32)
    PC = np.array(PC.points, dtype=np.float32)
    if np.shape(PC)[1] > 2:
        if PC.shape[0] > 3:
            PC = PC[0:3, :]
            pc_intensity = pc_intensity[:]
        if PC.shape[1] != model_inputsize:#go
            new_pts_idx = np.random.randint(
                low=0, high=PC.shape[1], size=model_inputsize, dtype=np.int64)
            PC = PC[:, new_pts_idx]
            pc_intensity=pc_intensity[new_pts_idx]
        PC = PC.reshape(3, model_inputsize)#PC.reshape(1, 3, model_inputsize)
        PC=np.vstack((PC, pc_intensity[np.newaxis,:]))

    else:
        PC = np.zeros((4, model_inputsize))#np.zeros((1, 3, model_inputsize))

    return PointCloud(PC)

def regularizePC(PC, model_inputsize):
    PC = np.array(PC.points, dtype=np.float32)
    if np.shape(PC)[1] > 2:
        if PC.shape[0] > 3:
            PC = PC[0:3, :]
        if PC.shape[1] != model_inputsize:
            new_pts_idx = np.random.randint(
                low=0, high=PC.shape[1], size=model_inputsize, dtype=np.int64)
            PC = PC[:, new_pts_idx]
        PC = PC.reshape(1, 3, model_inputsize)

    else:
        PC = np.zeros((1, 3, model_inputsize))
...
def getModel(PCs, boxes, offset=0, scale=1.0, normalize=False):
    ...
    if len(PCs) == 0: #可跟踪序列PC丢失的情况 PC为空
...
def cropPC(PC, box, offset=0, scale=1.0):
    ...
    if True in close:
        sample_PC = np.vstack((PC.points[:, close], PC.intensity[close]))
    else:
        sample_PC = np.zeros((4, 1))
