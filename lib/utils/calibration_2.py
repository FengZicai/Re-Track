...
class Calibration(object):
    def __init__(self, calib_file):
        ...
        self.V2C = calib['Tr_velo_cam']  # 3 x 4
        self.extrinsic = calib['extrinsic'] # 4 x 4
    ...
    def project_ego_to_cam(self, pts_3d_ego):
        ...
        uv_cam = self.extrinsic.dot(self.cart_to_hom(pts_3d_ego).transpose())

        return uv_cam.transpose()[:, 0:3]

    def project_cam_to_ego(self, pts_3d_rect):
        ...
        """
        return np.linalg.inv((self.extrinsic)).dot(self.cart_to_hom(pts_3d_rect).transpose()).transpose()[:, 0:3]
