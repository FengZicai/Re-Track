...
class KittiDataset(torch_data.Dataset):
    def __init__(self, root_dir, split='train'):
        ...
        self.imageset_dir = os.path.join(root_dir, 'KITTI', 'track', 'testing' if is_test else 'training')
    ...
    def get_lidar(self, idx, idy):
        lidar_file = os.path.join(self.lidar_dir, '%04d' % idy, '%06d.bin' % idx)
        assert os.path.exists(lidar_file)
        return np.fromfile(lidar_file, dtype=np.float32).reshape(-1, 4)

    def get_calib(self, idy):
        calib_file = os.path.join(self.calib_dir, '%04d.txt' % idy)
        assert os.path.exists(calib_file)
        return calibration.Calibration(calib_file)

    def get_label(self, idx, idy):
        label_file = os.path.join(self.label_dir, '%04d' % idy, '%06d.txt' % idx)
